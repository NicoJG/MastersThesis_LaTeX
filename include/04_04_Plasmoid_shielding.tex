...
Therefore, the shielding length can be estimated by eliminating the time dependence in the equation of motion\footnote{We assume a constant acceleration over the whole trajectory, which might not be true in reality.} and taking the absolute value, which gives
\begin{equation}
...
While increasing shielding length $s$, i.e. decreasing $\alpha$, attenuates the heat flux, the effective energy is enhanced.
...
Comparison to the definition of the heat flux asymmetry parameter then leads to
\begin{equation}
