...
While several more sophisticated models were developed, most models yield the same results \autocite{pegourie_review_2007}.
...
The asymmetric perturbation is then solved, as described in \cref{ssec:asymmetric_perturbation_model}, to predict the gas dynamics at the pellet surface, given the external heating source.

While this model can be used by itself, if the heat source into the neutral ablation cloud is known, our goal is to predict the pellet rocket acceleration along the major radius of a tokamak.
...
\end{equation*}
where the minus sign indicates that this is the force exerted on the pellet, while the surface element $\dd{S}$ points outwards.
\begin{figure}
    \centering
...
\begin{equation*}
    ...
    \end{cases} \, ,
\end{equation*}
which is helpful because ${Y_1^0=\cos\theta}$ appears in the surface integral.
